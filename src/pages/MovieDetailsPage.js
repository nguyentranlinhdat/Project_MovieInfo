...
import { fetcher, apiKey, tmdbAPI } from "../config/config";
...
const MovieDetailsPage = () => {
  ...
  return (
    <div className="py-10">
      ...
      {genres.length > 0 && (
        <div className="flex items-center justify-center mb-10 text-white gap-x-5">
          {genres.map((item) => (
            <span
              key={item.id}
              className="px-4 py-2 border rounded border-primary"
            >
              {item.name}
            </span>
          ))}
      ...
      <p className="text-center text-white max-w-[600px] mx-auto leading-relaxed mb-10">
        {overview}
      </p>
